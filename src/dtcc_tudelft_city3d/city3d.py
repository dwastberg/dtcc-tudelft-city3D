...
def generate_building_meshes(city: City, set_ground_to_zero=True):
    ...
    for building in city.buildings:
        pts = building.roofpoints.points
        ground_level = building.ground_level
        if (len(pts) < 6) or (
            building.roofpoints.points[:, 2].max()
            - building.roofpoints.points[:, 2].min()
            < 0.1
        ):
            print(
                "Building has less than 6 points or a flat roof using simple extrusion"
            )
            building.mesh = builder.meshing.extrude_building(
                building, ground_to_zero=set_ground_to_zero
            )
            continue
        ...
        succ = _dtcc_tudelft_city3d.mesh_building(
            pts, footprint, ground_level, tmp_file
        )
        if succ:
            mesh = io.load_mesh(tmp_file)
            building.mesh = mesh.triangulate()
        else:
            print(
                "Failed to generate mesh for building",
                building.uuid,
                "using simple extrusion",
            )
            building.mesh = builder.meshing.extrude_building(
                building, ground_to_zero=set_ground_to_zero
            )
